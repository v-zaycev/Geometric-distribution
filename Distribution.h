 ...
 *
 * Набор классов, позволяющий хранить информацию о гипергеометрических распределелениях, моделировать выборки для заданного распределения различными методами, и вычислять различные 
 значения, необходимые для проверки гипотез, с помощью критерия хи-квадрат. <br> Доступны три метода моделирования:
...
метод обратной функции, метод испытаний Бернулли и табличный метод обратной функции. Класс позволяет получить полную информацию о выборке и методе моделирования.  
*/
...
/*!
	...
	\brief	Класс, реализующий вычисления различных значений, используемых при проверке гипотез, с помощью критерия хи-квадрат

	Данный класс позволяет считать значение p-value для заданной гипотезы и выборки, и на основе этого может получать распределение p-value, значение мощности для выборки p-value и 
	строить зависимость мощности от размера выборки. Все получаемые значения хранятся в классе и могут быть получены при необходимости.
*/
class Chi_sq
{
private:
